...
while running:
    ...
    if keys[pygame.K_LEFT]:
        image_jugador_rect.x -= move_speed_3
    if keys[pygame.K_RIGHT]:
        image_jugador_rect.x += move_speed_3
    if keys[pygame.K_UP]:
        image_jugador_rect.y -= move_speed_3
    ...
    if any(obstacle.colliderect(image_jugador_rect) for obstacle in obstaculos) and not perdiendo:
