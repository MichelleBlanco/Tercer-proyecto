...
with open('preguntas_ciudad.json', 'r') as file:
    preguntas_ciudad = json.load(file)
...
class ArbolPreguntas:
    def __init__(self, preguntas):
        self.preguntas = sorted(preguntas, key=lambda x: x['peso'])  # Ordenar preguntas por peso
    ...
    def jugar(self):
        ...
        with open("preguntas_ciudad.json", "w") as file:
            json.dump(self.preguntas, file, indent=4)
        
        modos_juego = ["bosque", "mar", "montaña", "espacio"]
        modo_seleccionado = random.choice(modos_juego)
        if modo_seleccionado == "bosque":
...
# Ejecutar el juego
arbol = ArbolPreguntas(preguntas_ciudad)
arbol.jugar()
